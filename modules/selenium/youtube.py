...
class SeleniumYouTube(SeleniumWebsite):
    ...
    def scrape_url(self, url:str=None) -> dict:
        ...
        res['title'] = WebDriverWait(self.driver, 10).until(
        ...
        res['duration'] = self.driver.find_element_by_class_name("ytp-time-duration").text
        
        res['author'] = self.driver.find_element_by_xpath("//yt-formatted-string[contains(@class, 'ytd-channel-name')]/a").get_attribute('href')
        
        res['date'] = self.driver.find_element_by_xpath("//div[@id='info-strings']/yt-formatted-string").text
        
        description_child_elements = self.driver.find_elements_by_xpath("//div[@id='description']/yt-formatted-string/*")
        res['description'] = []
        for element in description_child_elements:
            if element.tag_name == 'span':
                ...
                raise ValueError("Unknown description element tag: %s" % element.tag_name)
