...
class SeleniumBestBuy(SeleniumWebsite):
    ...
    def login(
        ...
        if stay_signed_in: self.driver.find_element_by_class_name('c-checkbox-input').click()
        self.driver.find_element_by_xpath("//button[@type='submit']").click()
        
        # Skip prompt to update recovery phone
        wait.until(EC.presence_of_element_located((By.ID,'recoveryPhone')))
        self.driver.find_element_by_xpath("//button[contains(@class,'cia-cancel')]").click()
    ...
    def scrape_url(self, url:str=None) -> dict:
        ...
        res['productTitle'] = self.driver.find_element_by_xpath("//div[contains(@class,'sku-title')]/h1").text
        res['sitePrice'] = self.driver.find_element_by_class_name('priceView-customer-price').text
        # res['availability']
        # res['rating']
        # res['imageURL']
